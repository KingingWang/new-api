...
func TestShouldRetryEmptyAndBlacklistedResponseErrors(t *testing.T) {
	...
	require.True(t, service.ShouldRetryRelayError(c, emptyErr, 1), "empty response errors must retry on another channel")
	require.True(t, service.ShouldRetryRelayError(c, blacklistedErr, 1), "blacklisted response errors must retry on another channel")

	assert.False(t, service.ShouldRetryRelayError(c, emptyErr, 0), "no retries left")
	...
	assert.False(t, service.ShouldRetryRelayError(c, emptyErr, 1), "pinned channel must not retry")
}
