...
// MigrateColumnUnique 与 gorm 核心实现保持一致,仅在删除唯一约束前校验
// 规范命名的约束确实存在。表名必须经由 RunWithValue 解析:它携带调用
// Session 上的 Table() 指定的表名,直接 stmt.Parse(value) 会退回到
// 模型默认命名,导致约束名与实际表不匹配。
func (m guardedUniqueMigrator) MigrateColumnUnique(value interface{}, field *schema.Field, columnType gorm.ColumnType) error {
	...
	// 对齐 gorm 核心 RunWithValue:携带 Session 上的 Table()/TableExpr
	// 指定的表名,再解析模型;直接 Parse(value) 会退回模型默认命名,
	// 唯一约束名与实际表不匹配。
